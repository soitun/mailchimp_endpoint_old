...
describe MailChimpEndpoint do
  ...
  it 'considers already subscribed a success' do
    @mailchimp_list.should_receive(:subscribe)
    .with('f167c0d6ca', 'andrew@spreecommerce.com', hash_including('FNAME' => 'Andrew'))
    .and_return({ "error"=>"andrew@spreecommerce.com is already subscribed to list Test.",
                  "code" => 214})

    post '/subscribe', message.to_json, auth
    last_response.status.should == 200
    last_response.body.should match /andrew@spreecommerce\.com is already subscribed to list Test/
  end
